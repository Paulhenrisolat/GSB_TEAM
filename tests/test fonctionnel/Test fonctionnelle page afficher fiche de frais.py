...
# selection mois
try:
    driver.get("http://gsbproject/index.php?uc=etatFrais&action=selectionnerMois")
    select = Select(driver.find_element_by_id('lstMois'))
    select.select_by_visible_text('01/2021')
except Exception as ex:
    log("- Erreur lors du select des mois")
    print(ex)
...
try:
    ...
    log("- Erreur lors de la validation du mois")
    print(ex)
